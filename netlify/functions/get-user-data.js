...
exports.handler = wrap(async (event) => {
  ...
  const [tickets, cards] = await Promise.all([
    sql`SELECT ticket_id, draw_id, number, price_pi, status, is_winner, expires_at, network, created_at
        FROM tickets
        WHERE uid = ${user.uid} AND network = ${network}
        ORDER BY created_at DESC LIMIT 200`,
    sql`SELECT card_id, status, price_pi, reward_pi, scratched_at, network, created_at
        FROM cards
        WHERE uid = ${user.uid} AND network = ${network}
        ORDER BY created_at DESC LIMIT 200`,
  ]);

  return ok({ user, tickets, cards, network });
});
